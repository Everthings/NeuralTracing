...
with tf.Session() as sess:
	...
	lr = float(argv[2])
	print('Init Train')
	fu = FbUpTrainBN2D(sess, input_placer, gold_placer, keep_prob,
					learn_rate_placer, wd = wd)
	#variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope = "fg_"+str(30)+'_'+str(30))
	fu.saver = tf.train.Saver()

	print('Train')
	fu.train(epic_num = 1,keep_prob = 1.0, batch_size = 64,
		learning_rate = lr)
	fu.saver.save(fu.sess,'./model/2d/version_'+'0'+'/model_foreground_'+str(30)+'.ckpt')
