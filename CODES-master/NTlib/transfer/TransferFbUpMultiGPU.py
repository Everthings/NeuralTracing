...
class TransferFbUpTrain(object):
	...
	def get_nn(self,input_placer,gold_placer,learn_rate_placer,keep_prob,window_size,
		...
		with tf.variable_scope("transfer"):
			...
			with tf.variable_scope("transfer_final"):
				W_fc1 = self.nn.weight_variable([1,1,1,conv_5_neuron_num,2],wd = wdo)
		...
		if self.valid_flag:
			...
			cross_entropy = tf.add_n(tf.get_collection('losses',scope), name='total_loss')
	...
	def loss(self,opt,scope):
		...
		self.train_step = opt.minimize(cross_entropy, var_list = transfer_vars)
		correct_prediction = tf.equal(tf.argmax(self.y_conv,1), tf.argmax(self.y_,1))
		self.accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
	...
	def train_steps(self, gpu_num):
		 with tf.Graph().as_default(), tf.device('/cpu:0'):
		 	opt = tf.train.AdamOptimizer(self.l_rate)
		 	tower_grads = []
		 	for i in xrange(gpu_num):
		 		 with tf.device('/gpu:%d' % i):
		 		 	with tf.name_scope('%s_%d' % ('Tower_', i)) as scope:
		 		 		loss = self.loss(scope)

	def train(self, epic_num = 8,loop_size = 2100,batch_size = 32,learning_rate = 0.01, thres = 0.01,keep_prob = 1.0, tao = 50, gpu_num = 2):
