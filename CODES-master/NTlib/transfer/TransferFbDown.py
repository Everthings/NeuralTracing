	...
	def get_nn(self,input_placer,gold_placer,learn_rate_placer,keep_prob,window_size,
		...
		for angle_xy in range(30,181,30):
			for angle_xz in range(30,181,30):
				...
				#	fg_dict[(angle_xy,angle_xz)].para_dict['h_conv2'])
		FEAT_NUM = 32
		CONV2_NUM = 8
		self.fg_dict = fg_dict
		y_conv_combined = tf.concat(4,y_conv_combined)
		#up_conv2_combined = tf.concat(4,up_conv2_combined)
		origin_shape = tf.shape()#tf.shape(up_conv2_combined)
		shape = tf.shape(y_conv_combined)
		#dir_out = tf.reshape(y_conv_combined,[shape[0],shape[1],shape[2],shape[3],36 * FEAT_NUM])
		...
		with tf.variable_scope("transfer"):
			with tf.variable_scope("up_conv2_layer1"):
				...
				h_up_layer1 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(input_placer, W_up_layer1, padding = padding) , b_up_layer1))
			...
			with tf.variable_scope("up_conv2_layer2"):
				...
				h_up_layer2 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_up_layer1, W_up_layer2, padding = padding) , b_up_layer2))
			...
			with tf.variable_scope("up_conv2_layer3"):
				...
				h_up_layer3 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_up_layer2, W_up_layer3, padding = padding) , b_up_layer3))
			...
			with tf.variable_scope("transfer_layer_1"):
				...
				h_conv1 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(dir_out, W_conv1, padding = padding) , b_conv1))	
			...
			with tf.variable_scope("transfer_layer_2"):
				...
				h_conv2 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_conv1, W_conv2, padding = padding) , b_conv2))	
			...
			with tf.variable_scope("transfer_layer_4"):
				W_conv4 = self.nn.weight_variable([conv_4,conv_4,conv_4,2*conv_3_neuron_num,conv_4_neuron_num],wd,conv_3 ** 3 * 2*conv_3_neuron_num,conv_4 ** 3 * conv_4_neuron_num)
				b_conv4 = self.nn.bias_variable([conv_4_neuron_num])
				h_conv4 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(up_down_combine, W_conv4, padding = padding) , b_conv4))
			...
			with tf.variable_scope("transfer_layer_5"):
				...
				h_conv5 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_conv4, W_conv5, padding = padding) , b_conv5))	
			...
			with tf.variable_scope("transfer_layer_6"):
				...
				h_conv6 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_conv5, W_conv6, padding = padding) , b_conv6))

			weight_decay = tf.mul(tf.nn.l2_loss(h_conv6), wdo, name='weight_loss')
			tf.add_to_collection('losses', weight_decay)

			with tf.variable_scope("transfer_final"):
				W_fc1 = self.nn.weight_variable([1,1,1,conv_6_neuron_num,2],wd)
				b_fc1 = self.nn.bias_variable([2])
				h_fc1 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_conv6, W_fc1, padding = padding) , b_fc1))
		...
		if self.predict_flag:
			...
			self.y_conv_2x = self.y_conv#dilation3D(self.y_conv)
			tmp = tf.expand_dims(self.y_res,4)
			self.y_res_2x = tf.squeeze(tmp)#tf.squeeze(dilation3D(tmp))
