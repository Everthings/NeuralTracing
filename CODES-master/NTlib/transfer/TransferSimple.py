...
class TransferSimple(object):
	...
	def get_nn(self,input_placer,gold_placer,learn_rate_placer,keep_prob,window_size,
		...
		for angle_xy in range(30,181,30):
			for angle_xz in range(30,181,30):
				...
				y_conv_combined.append(
					tf.expand_dims(fg_dict[(angle_xy,angle_xz)].para_dict['h_conv4'],5))
		...
		with tf.variable_scope("transfer"):
			...
			weight_decay = tf.mul(tf.nn.l2_loss(h_conv5), wdo, name='weight_loss')
				...
				if i%10 == 0:
					...
					for v_idx in range(0,len(valid[0]),256):
						...
						dir_out = self.dir_out.eval(session = self.sess, feed_dict={self.x: valid[0][v_idx:min(v_idx+256,len(valid[0]))]/255.0,
											   											...
											   											self.keep_prob: 1.0,
											   											self.l_rate: learning_rate})
						print(np.max(dir_out),np.min(dir_out))
	...
	def predict_large(self,image,batch_diameter = 64, extra_edge_width = 15):
		...
		for x_start,y_start,z_start,batch in image3D_crop_generator(image,batch_diameter,extra_edge_width):
			...
			tmp_cov = tmp_cov[0,extra_edge_width:-extra_edge_width,
								extra_edge_width:-extra_edge_width,
								extra_edge_width:-extra_edge_width,0]
		...
		misc.imsave('test_max0.png',np.max(max_res[10:-10,10:-10,10:-10],axis = 0)*255)
		misc.imsave('test_max1.png',np.max(max_res[10:-10,10:-10,10:-10],axis = 1)*255)
		misc.imsave('test_max2.png',np.max(max_res[10:-10,10:-10,10:-10],axis = 2)*255)
		misc.imsave('test_cov0.png',np.max(result[10:-10,10:-10,10:-10],axis = 0)*255)
		misc.imsave('test_cov1.png',np.max(result[10:-10,10:-10,10:-10],axis = 1)*255)
		misc.imsave('test_cov2.png',np.max(result[10:-10,10:-10,10:-10],axis = 2)*255)
