...
import time
import pickle
class DNForegroundTrain(object):
	...
	def get_nn(self,input_placer,gold_placer,learn_rate_placer,
		...
		with tf.variable_scope("fs_train_nn"):
			self.para_dict = self.nn.generate_flow(self.x,self.keep_prob,wd = wd, wdo = wdo)
			h_fc1 = self.para_dict['h_fc1']
		with tf.variable_scope("fs_fc2"):
			W_fc2 = self.nn.weight_variable([fc_1_neuron_num, 2])
