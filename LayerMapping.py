...
class LayerMapping():
    ...
    # use l2, return float
    TIFF = TIFFExtractor().extract("neuron-data/data1_input.tif")
    SWC = SWCExtractor().extract((10, 1024, 1024), "neuron-data/data1_label.swc")
    ...
    def map(self, tiff_filepath, swc_filepath):
        ...
        sol = minimize(self.objective, x0, method='    ', bounds=bounds)
        ...
        for z in range(mat.shape[0]):
            ...
            for x in range(mat.shape[1]):
                for y in range(mat.shape[2]):
                    if mat[z][x][y] > 0:
                        mapped[int(index_lower)][x][y] = min(mapped[int(index_lower)][x][y] + intensity[1], 255)
                        mapped[int(index_upper)][x][y] = min(mapped[int(index_upper)][x][y] + intensity[2], 255)
        return mapped
