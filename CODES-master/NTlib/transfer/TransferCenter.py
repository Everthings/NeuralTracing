...
class TransferCenter(object):
	...
	def get_nn(self,input_placer,gold_placer,learn_rate_placer,keep_prob,window_size,
		...
		if self.predict_flag:
			dir_out = dilation3D(y_conv_combined,[0,13,13,13,0])
			cropped_x = input_placer[:,4:19,4:19,4:19,:]
		else:
			dir_out = dilation3D(y_conv_combined,tf.shape(input_placer))
			cropped_x = input_placer
		...
		with tf.variable_scope("transfer"):
			...
			with tf.variable_scope("up_layer_1"):
				W_conv_up1 = self.nn.weight_variable([up_conv_1, up_conv_1, up_conv_1,
													  1, up_conv_1_neuron_num],
													  wd,
													  1,
													  up_conv_1 ** 3 * up_conv_1_neuron_num)
				b_conv_up1 = self.nn.bias_variable([up_conv_1_neuron_num])
				h_conv_up1 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(cropped_x, W_conv_up1, padding = padding) , b_conv_up1))
			...
			h_conv1 = tf.concat(4,[h_conv1,h_conv_up1])

			with tf.variable_scope("transfer_layer_2"):
				W_conv2 = self.nn.weight_variable([conv_2,conv_2,conv_2,2*conv_1_neuron_num,conv_2_neuron_num],wd,conv_1 ** 3 * 2*conv_1_neuron_num, conv_2 ** 3 * conv_2_neuron_num)
			...
			with tf.variable_scope("transfer_final"):
				W_fc1 = self.nn.weight_variable([1,1,1,conv_7_neuron_num,2])
				b_fc1 = self.nn.bias_variable([2])
				h_fc1 = tf.nn.relu(tf.nn.bias_add(self.nn.conv2d(h_conv7, W_fc1, padding = padding) , b_fc1))
		
		if self.predict_flag:
			final_shape = tf.shape(h_fc1)
			self.final_shape = final_shape
			#self.y_conv = tf.nn.softmax(h_fc1[:,(final_shape[1]-1)/2,(final_shape[2]-1)/2,(final_shape[3]-1)/2,:])
			self.y_conv = tf.nn.softmax(h_fc1[:,1,1,1,:])
			...
			tmp = self.y_res#tf.expand_dims(self.y_res,4)
